...
export const HoverEffect = ({
  ...
  return (
    <div
      className={cn(
        "grid grid-cols-1 gap-6 py-10 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4",
        className,
      )}
    >
      {items.map((item, idx) => (
        ...
        >
          <AnimatePresence>
            {hoveredIndex === idx && (
              <NeonGradientCard className="absolute inset-0 -z-30 block h-full w-full rounded-3xl border border-white border-opacity-20 bg-white bg-opacity-10 backdrop-blur-md">
                <motion.span
                  className="absolute inset-0 block h-full w-full rounded-3xl bg-white bg-opacity-20"
                  layoutId="hoverBackground"
                  initial={{ opacity: 0 }}
                  animate={{
                    opacity: 1,
          ...
          <div className="rounded-md p-6 ring-1 ring-gray-200 dark:ring-gray-700">
            <Feature
              title={item.title}
              description={item.description}
              IconComponent={item.IconComponent}
...
const Feature: React.FC<FeatureProps> = ({
  ...
  <div className="flex flex-col items-center rounded-lg p-8 px-6 text-black shadow-lg backdrop-blur-md dark:bg-white/10 dark:text-white">
